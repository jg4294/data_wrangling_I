...
```{r}
arrange(litters , pups_born_alive) #show the output based on the order of pup-born_alive, from small to large
```

```{r}
arrange(litters , pups_born_alive, gd0_weight)# first order by pups_born_alive, and then, witin that, order each level by gd0_weight
```
