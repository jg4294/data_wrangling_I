...
Select helper: ?select\_helpers (type in the console)
...
It’s intended to remove rows or keep rows that u are interested in
retainning for some subsequent analysis.So select every single row every
time we did, it just reduced or changed or reorganized the columns.
Select operates on columns filter now is gonna operate on rows.
...
It allows us either to modify an exising variable or create some new
variable
